...
app_server <- function(input, output, session) {
  ...
  ccg_sel <- mod_region_antibiotic_server("region_antibiotic_1", metric_sel)
  ccg_selected <- mod_ccg_selection_server("ccg_selection_1", ccg_sel)
  # ccg_sel <- mod_ccg_selection_server("ccg_selection_1")
  mod_gp_overall_server("gp_overall_1", metric_sel, ccg_selected)
  gp_val <- mod_gp_overall_server("gp_overall_1", metric_sel, ccg_selected)
